...
def measurement_update(sensor_var, p_cov_check, y_k, p_check, v_check, q_check):
    ...
    S = np.add(h_jac@p_cov_check@h_jac.T,R)
    KK = p_cov_check@h_jac.T@np.linalg.inv(S)
    # 3.2 Compute error state
    delta_x = KK@(y_k-p_check)
    # 3.3 Correct predicted state
    p_hat = p_check + delta_x[0:3]
    v_hat = v_check + delta_x[3:6]
    q_Phi = Quaternion(axis_angle = angle_normalize(delta_x[6:9]))
    q_hat = Quaternion(*q_check).quat_mult_left(q_Phi)
    # 3.4 Compute corrected covariance
    p_cov_hat = np.dot(np.subtract(np.eye(9),np.dot(KK,h_jac)),p_cov_check)
...
for k in range(1, imu_f.data.shape[0]):  # start at 1 b/c we have initial prediction from gt
    ...
    p_check = p_est[k-1].reshape(3, 1) + delta_t * v_est[k-1].reshape(3,1) + ((delta_t**2)/2) * np.add(np.dot(C_ns, imu_f.data[k-1].reshape(3,1)), g.reshape(3,1))
    v_check = v_est[k-1].reshape(3, 1) + delta_t * np.add(np.dot(C_ns, imu_f.data[k-1].reshape(3,1)), g.reshape(3,1))
    theta = delta_t * imu_w.data[k-1]
    q_theta = Quaternion(axis_angle = angle_normalize(theta))
    q_k = Quaternion(*q_est[k-1]).quat_mult_right(q_theta)  
    q_check = q_k.reshape(4,1)
    ...
    p_cov_check = F_k@p_cov[k-1]@F_k.T + l_jac@Q@l_jac.T
    # 3. Check availability of GNSS and LIDAR measurements
    count = 0
    for i in range(0, gnss.t.shape[0]):
        t_gnss = gnss.t[i]
        if t_gnss == imu_f.t[k]:
            y_k = gnss.data[i].reshape(3,1)
            sensor_var = var_gnss
            count = count+1
            break
    if count != 0:
        p_hat, v_hat, q_hat, p_cov_hat = measurement_update(sensor_var, p_cov_check, y_k, p_check, v_check, q_check)
        p_est[k] = p_hat.reshape(1,3)
        v_est[k] = v_hat.reshape(1,3)
        q_est[k] = q_hat.reshape(1,4)
        p_cov[k, :, :] = p_cov_hat
    if count == 0:
        for i in range(0, lidar.t.shape[0]):
            t_lidar = lidar.t[i]
            if t_lidar == imu_f.t[k]:
                y_k = lidar.data[i].reshape(3,1)
        ...
        if count != 0:
            ...
            p_cov[k, :, :] = p_cov_hat
        if count == 0:
            p_est[k] = p_check.reshape(1,3)
            v_est[k] = v_check.reshape(1,3)
            q_est[k] = q_check.reshape(1,4)
            p_cov[k, :, :] = p_cov_check 
